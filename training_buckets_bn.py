...
import glob
import pickle
from batch_norm_model import full_conv
import sys
import os
import time
import tf_utils
...
from get_input import get_number_of_steps
...
if args.retrain:
    runs_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "runs_groups"))
    candidates = []
...
training_sizes = set_batch_size(classes=n_labels, memory_limit=13, _size_boundaries=training_size_boundaries,
...
validation_sizes = set_batch_size(classes=n_labels, memory_limit=13, _size_boundaries=validation_size_boundaries,
...
#                                       batch_sizes=batch_sizes, data=training_data, number_of_gpus=4)
#
...
#                                     batch_sizes=batch_sizes, data=validation_data, number_of_gpus=4)
# parameters['steps_to_val'] = int(train_n_iter / 2)


def training_model(input_fn, n_outputs, is_training):
    image_batch, labels_batch = input_fn()
    image_batch = tf.cast(image_batch, dtype=tf.float16)

    dropout_rate = tf.placeholder(tf.float16, name="drop_rate")
    #phase_train = tf.placeholder(tf.bool,name="is_train")
    with tf.name_scope("model"):
        model = full_conv(n_outputs=n_outputs, dropout_rate=dropout_rate,is_train=is_training)
    ...
    # train_op = tf.group(opt_step, update_ops)

    #return train_op,batch_loss, predictions, labels_batch, dropout_rate
    return batch_loss, predictions, labels_batch, dropout_rate


def create_parallel_optimization(model_fn, input_fn, n_outputs, is_training, controller="/cpu:0"):
    ...
    with tf.variable_scope(tf.get_variable_scope()) as outer_scope:
        for i, id in enumerate(devices):
            ...
            with tf.device(tf_utils.assign_to_device(id, controller)), tf.name_scope(name):
                ...
                loss, preds, labels, dropout_tensor= model_fn(input_fn, n_outputs,is_training)
                with tf.name_scope("compute_gradients"):
                    ...
                    batchnorm_updates = tf.get_collection(tf.GraphKeys.UPDATE_OPS,scope=name)
    ...
    return train_op, avg_loss, concat_preds, concat_labels, dropout_tensor
...
#train_op, batch_loss, predictions, labels, drop_rate, is_train =training_model(input_fn,n_labels)
...
model_dir = os.path.abspath(os.path.join(out_path, "model"))
    ...
    for epoch in range(parameters['max_epochs']):
        ...
        try:
            while True:
                ...
                t0 = time.time()
                feed_dict = {handle: training_handle, drop_rate:parameters['drop_rate'], phase_train: True}
        ...
        try:
            while True:
                ...
                # Validation step
                t0 = time.time()
                feed_dict = {handle: validation_handle, drop_rate: 0.0, phase_train: False}
