...
def spp_layer2(sp_tensor, levels=[4,2,1], name='SPP_layer'):
    ...
    with tf.variable_scope(name):
        pool_outputs = []
        for l in levels:
            pool = gen_nn_ops.max_pool_v2(sp_tensor,
                                          ksize=[1, tf.math.ceil(tf.math.divide(tf.shape(sp_tensor)[1], l)),
                                                 tf.math.ceil(tf.math.divide(tf.shape(sp_tensor)[2], l)), 1],
                                          strides=[1, tf.math.floor(tf.math.divide(tf.shape(sp_tensor)[1], l )),
                                                   tf.math.floor(tf.math.divide(tf.shape(sp_tensor)[2], l )),
                                                   1],
                                          padding='VALID')

            pool_outputs.append(tf.reshape(pool, [tf.shape(sp_tensor)[0], -1]))

        spp_pool = tf.concat(pool_outputs, 1)

        spp_pool = tf.reshape(spp_pool, (-1, 16* 256 + 4*256 +256))
...
def build_forward_model(input_tensor):
    ...
    top_layer = spp_layer2(conv4_1)


    #top_layer= tf.reduce_mean(pool4, axis=[2,3], name="global_pooling")

    #top_layer = tf.layers.dropout(global_average_pooling, rate=dropout_rate, name='global_dropout')
    ...
    for epoch in range(parameters['max_epochs']):
        ...
        try:
            while True:
                ...
                path = saver.save(sess, os.path.join(model_dir, 'model3'),
                          global_step=epoch,
